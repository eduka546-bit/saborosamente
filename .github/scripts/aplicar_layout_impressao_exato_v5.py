...
ini = s.index('  return <Janela titulo={`Ficha de produção total do dia — ${dataFmt}`}')
fim = s.index('\n}\n\nfunction FichaMontagemModal', ini)
novo_dia = r'''  return <Janela titulo={`Ficha de produção total do dia — ${dataFmt}`} fechar={fechar}>
...
s = s[:ini] + novo_dia + s[fim:]

ini = s.index('  return <Janela titulo={`Ficha de produção — ${produto.nome}`}')
fim = s.index('\n}\nfunction IngredienteModal', ini)
novo_prod = r'''  return <Janela titulo={`Ficha de produção — ${produto.nome}`} fechar={fechar}>
