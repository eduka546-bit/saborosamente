...
old = '''  const telefoneNormalizado = String(conversa.telefone ?? "").replace(/\\D/g, "");
...
'''
new = '''  const telefoneNormalizado = String(conversa.telefone ?? "").replace(/\\D/g, "");
  const telefoneBusca = telefoneNormalizado.startsWith("55") ? telefoneNormalizado.slice(2) : telefoneNormalizado;
...
'''
if old not in s:
    raise SystemExit('anchor contexto telefone nao encontrado')
...
'''
if old not in s:
    raise SystemExit('select pedidos nao encontrado')
s = s.replace(old, new, 1)

old = '''  const totalGastoCliente = pedidosValidos.reduce(
...
'''
new = '''  const totalGastoCliente = pedidosValidos.reduce(
...
'''
if old not in s:
    raise SystemExit('anchor resumo pedidos nao encontrado')
s = s.replace(old, new, 1)

old = '''  const mensagens: any[] = [...mensagensDaConversa, ...mensagensDeCampanha].sort(
  ...
  useEffect(() => {
'''
new = '''  const mensagens: any[] = [...mensagensDaConversa, ...mensagensDeCampanha].sort(
    (a, b) => new Date(a.timestamp ?? 0).getTime() - new Date(b.timestamp ?? 0).getTime(),
  );
...
'''
if old not in s:
    raise SystemExit('declaracoes duplicadas nao encontradas')
s = s.replace(old, new, 1)
...
'''
if old not in s:
    raise SystemExit('anchor antes das mensagens nao encontrado')
s = s.replace(old, new, 1)
