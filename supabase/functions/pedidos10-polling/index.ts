...
async function getToken(): Promise<string> {
  ...
  const url = `${OD_BASE_URL}/oauth/token`;
  ...
  console.log(`Auth attempt: ${url} | client_id=${OD_CLIENT_ID.slice(0, 8)}...`);
  ...
  if (!res.ok) {
    throw new Error(`Auth falhou (${res.status}): ${text}`);
  }

  const data = JSON.parse(text);
  if (!data.access_token) {
    throw new Error(`Auth: resposta sem access_token: ${text}`);
  }
