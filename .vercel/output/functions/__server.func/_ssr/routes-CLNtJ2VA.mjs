...
import { i as isNoDiscount, t as COMBO_RULES } from "./combo-rules-C89YNLx0.mjs";
import { i as useCart, r as formatBRL } from "./cart-Dfy30lDm.mjs";
import { t as DiscountProgressWidget } from "./discount-progress-widget-CdTXxtVD.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/routes-CLNtJ2VA.js
...
function ComboBuilderModal({ isOpen, onClose, combo, products }) {
	...
	const { totalQty, subtotal, discountPct, discount, total } = (0, import_react.useMemo)(() => {
		const totalQty = items.reduce((s, i) => s + i.quantity, 0);
		const subtotal = items.reduce((s, i) => s + i.preco * i.quantity, 0);
		const marmitaSubtotal = items.filter((i) => !isNoDiscountLocal(i.categoria)).reduce((s, i) => s + i.preco * i.quantity, 0);
		const discountPct = COMBO_RULES.find((r) => totalQty >= r.min)?.discount ?? 0;
