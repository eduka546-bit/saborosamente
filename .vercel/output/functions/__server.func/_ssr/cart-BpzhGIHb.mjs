...
import { n as getPublicProducts } from "./products.functions-DumvJucT.mjs";
import { n as isNoDiscount } from "./combo-rules-CAvABk5q.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/cart-BpzhGIHb.js
...
function useAbandonedCart({ lines, total, onExitIntent }) {
	...
	if (typeof window !== "undefined" && window.location.pathname.startsWith("/admin")) return { markConverted: async () => {} };
...
function CartProvider({ children }) {
	...
	const value = (0, import_react.useMemo)(() => {
		...
		if (selectedBairro) {
			...
			if (taxaItem) shipping = taxaItem.taxa;
		}
		if (subtotal === 0 || subtotal >= 999999) shipping = 0;
		if (selectedCity) {
			if (selectedCity.toLowerCase().includes("são bento do sul") && count >= RULES.MIN_ORDER_QUANTITY) shipping = RULES.SBS_DISCOUNTED_SHIPPING;
		}
		let discount = 0;
		const applicableRule = [...RULES.PROGRESSIVE_DISCOUNT].sort((a, b) => b.min - a.min).find((rule) => count >= rule.min);
		if (applicableRule) discount = detailed.filter((l) => !isNoDiscount(l.product.categoria ?? "")).reduce((acc, l) => acc + l.subtotal, 0) * applicableRule.discount;
		return {
			lines: detailed,
			count,
			subtotal,
