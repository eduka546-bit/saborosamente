    ...
    >
      <div className="flex items-center gap-2 mb-1">
        ...
        {onChange ? (
          <input
            value={no.titulo}
            onChange={(event) => onChange({ ...no, titulo: event.target.value })}
        ...
        {(no.tipo === "condicao" || (no.tipo === "mensagem" && no.config.texto)) && (
          <button onClick={() => onToggleExpand(no.id)} className="cursor-pointer flex-shrink-0">
      ...
      {isExpanded && (
        <div className="text-xs mt-2 pt-2 border-t opacity-75">
          {no.tipo === "mensagem" &&
            (onChange ? (
              <textarea
                ...
                onChange={(event) =>
                  onChange({ ...no, config: { ...no.config, texto: event.target.value } })
                }
          ...
          {no.tipo === "aguardar" && (
            <p>
              {no.config.valor || 1} {no.config.unidade || "horas"}
            </p>
          )}
          {no.tipo === "tag" && (
            <p>
              Tag: <strong>{no.config.tag || "(vazia)"}</strong>
            </p>
          )}
          {no.tipo === "condicao" &&
            (onChange ? (
              <div className="space-y-1">
                <select
                  ...
                  onChange={(event) =>
                    onChange({ ...no, config: { ...no.config, campo: event.target.value } })
                  }
                ...
                <input
                  ...
                  onChange={(event) =>
                    onChange({ ...no, config: { ...no.config, valor: event.target.value } })
                  }
              ...
              <p>
                Se {no.config.campo} = {no.config.valor}
              </p>
            ))}
          {no.tipo === "menu" &&
            (onChange ? (
...
export function FlowDiagram({
  ...
  const [expandido, setExpandido] = useState<Set<string>>(
    new Set(nos.slice(0, 1).map((n) => n.id)),
  );
  ...
  const conexoes = nos.flatMap((no, indice) => {
    const destinos = [no.proximo_id, no.proximo_sim_id, no.proximo_nao_id].filter(
      Boolean,
    ) as string[];
    return destinos
      .map((destino, ramo) => ({
    ...
    <div className="w-full bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border border-gray-200 overflow-hidden">
      <div className="flex items-center justify-between gap-2 border-b bg-white/80 px-3 py-2">
        <div>
          ...
          <p className="text-[10px] text-gray-400">
            Arraste o espaço para navegar e organize os nós abaixo
          </p>
        </div>
        <div className="flex items-center gap-1">
          ...
          <span className="w-12 text-center text-xs font-semibold text-gray-500">
            {Math.round(zoom * 100)}%
          </span>
          <button
            type="button"
      ...
      {onGatilhoChange && (
        <div className="border-b bg-white p-3">
          <div className="mx-auto max-w-[520px] rounded-xl border-2 border-emerald-200 bg-emerald-50 p-3">
            <div className="mb-2 flex items-center gap-2 text-xs font-bold text-emerald-700">
              <Zap size={14} /> Gatilho inicial
            </div>
            <select
              value={gatilhoTipo}
            ...
            >
              ...
              <option value="primeira_msg">1ª mensagem</option>
              <option value="pedido_criado">Pedido criado</option>
              <option value="status_pedido">Status do pedido</option>
          ...
          <div className="mx-auto h-8 w-0.5 bg-slate-300" />
        </div>
      )}
      {nos.length === 0 ? (
        ...
        <div className="flex h-[520px] overflow-hidden">
          {onInsert && (
            <aside className="z-10 hidden w-44 shrink-0 border-r bg-white/90 p-3 md:block">
              <p className="mb-3 text-[10px] font-bold uppercase tracking-wide text-gray-500">
                Ações
              </p>
              <div className="space-y-1">
                {TIPOS_INSERCAO.map(([tipo, label, Icon]) => (
                  <button
                    key={tipo}
                    type="button"
          ...
          <div
            ...
            onWheel={(event) => {
              ...
              alterarZoom(event.deltaY > 0 ? -0.05 : 0.05);
            }}
          >
            ...
            <div
              ...
              style={{
                transform: `translateX(calc(-50% + ${pan.x / zoom}px)) translateY(${pan.y / zoom}px) scale(${zoom})`,
              }}
            >
              <svg
              ...
              >
                {conexoes.map((conexao, indice) => {
                  ...
                  return (
                    <g key={`${conexao.de}-${conexao.para}-${indice}`}>
                      <line
                        x1="150"
                        y1={inicioY}
                        x2="150"
                        ...
                        stroke="#94a3b8"
                        strokeWidth="2"
                        strokeDasharray={conexao.ramo ? "5 4" : undefined}
                      />
                      <text x="158" y={(inicioY + fimY) / 2} fill="#64748b" fontSize="10">
                        {conexao.ramo}
                      </text>
                    </g>
                  );
                    ...
                    {nos.map((no, indice) => (
                      <div key={no.id} className="flex flex-col items-center">
                        <NoDraggable
                          ...
                          onChange={(alterado) =>
                            onUpdate?.(
                              nos.map((item) => (item.id === alterado.id ? alterado : item)),
                            )
                          }
                          ...
                          <div className="relative flex h-12 items-center justify-center">
                            ...
                            {onInsert && (
                              <button
                                type="button"
                                onClick={() =>
                                  setInserindoEntre(inserindoEntre === indice ? null : indice)
                                }
