...
function AdminConfigHorariosPage() {
  ...
  const { isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      setHorarios(Array.isArray(h) && h.length === 7 ? h.map((d: any, i: number) => ({
        ...DEFAULT_HORARIOS[i], ...d
      })) : DEFAULT_HORARIOS);
      return data;
    },
  });
    ...
    <div className="p-4 md:p-6 max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">Configure atendimento e entregas por dia da semana.</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-xl border overflow-hidden">
          <div className="px-6 py-3 bg-gray-50 border-b flex items-center gap-2 text-xs font-bold text-gray-400 uppercase tracking-wider">
            {aba === "atendimento" ? <><Clock size={14} /> Horário de atendimento ao cliente</> : <><Truck size={14} /> Dias e horários de entrega</>}
          </div>
          {horarios.map((h, idx) => (
