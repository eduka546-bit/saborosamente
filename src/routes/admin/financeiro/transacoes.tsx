  ...
  const { data: orders = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("pedidos")
        .select(
          "id, nome_cliente, valor_total, taxa_entrega, desconto_aplicado, metodo_pagamento, status, created_at",
        )
  ...
  return (
    <div className="p-4 md:p-6 max-w-[1600px] mx-auto">
      ...
      <div className="bg-white rounded-xl border p-4 mb-6 flex gap-3 flex-wrap">
        ...
        >
          {["Todos", "pendente", "preparando", "saiu para entrega", "entregue", "cancelado"].map(
            (s) => (
            ...
            <tbody className="divide-y">
              ...
              {filtered.map((o: any) => (
                <tr key={o.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 font-mono text-xs text-[#5850ec]">
                    #{o.id.slice(0, 8)}
                  </td>
                  ...
                  <td className="px-6 py-4 text-red-500">
                    {o.desconto_aplicado ? `- R$ ${Number(o.desconto_aplicado).toFixed(2)}` : "—"}
                  </td>
