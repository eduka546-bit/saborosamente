...
function AdminCuponsNovoPage() {
  ...
  const [form, setForm] = useState({ codigo: "", tipo: "Fixo", valor: "", regra: "", validade: "", ativo: true });
  ...
  return (
    <div className="p-4 md:p-6 max-w-xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm mt-1">Preencha os dados para criar um novo cupom de desconto.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl border p-6 space-y-5">
        <div>
          ...
          <Input value={form.codigo} onChange={e => setForm({ ...form, codigo: e.target.value.toUpperCase() })} required placeholder="EX: SABOR20" className="uppercase font-bold" />
        </div>
        ...
        <div>
          <label className="text-xs font-bold uppercase text-gray-400 mb-1 block">Regra / Condição</label>
          <Input value={form.regra} onChange={e => setForm({ ...form, regra: e.target.value })} placeholder="Ex: Mínimo R$ 100" />
        </div>
        <div>
        ...
        <div className="flex gap-3 pt-2">
          <Button type="button" variant="outline" onClick={() => navigate({ to: "/admin/cupons" })} className="flex-1">Cancelar</Button>
          <Button type="submit" disabled={!form.codigo || saveMutation.isPending} className="flex-1 bg-[#5850ec] text-white">
            {saveMutation.isPending ? <Loader2 size={16} className="animate-spin mr-2" /> : null} Criar Cupom
          </Button>
        </div>
      </form>
    </div>
