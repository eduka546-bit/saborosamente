...
type Aba = "producao" | "ingredientes" | "receitas" | "estoque";
...
function CozinhaPage() {
  ...
  const [abaEstoque, setAbaEstoque] = useState<"loja" | "cozinha">("loja");
  ...
  const abas: { id: Aba; label: string; icon: any }[] = [
    { id: "producao", label: "Produção", icon: ClipboardList }, { id: "ingredientes", label: "Hoje vamos usar", icon: Salad }, { id: "receitas", label: "Receitas", icon: BookOpen }, { id: "estoque", label: "Estoque", icon: Package },
  ];
  ...
  return <div className="min-h-screen bg-[#f7f6f0] text-[#173a2d]">
    ...
    <div className="mx-auto grid max-w-7xl md:grid-cols-[220px_1fr]">
      ...
      <main className="p-4 md:p-8">
        ...
        {aba === "estoque" && <section><Titulo titulo="Estoque geral" texto="Consulte o saldo de marmitas da loja e controle os ingredientes da cozinha no mesmo lugar."/><div className="mb-5 inline-flex rounded-xl bg-[#e7eee8] p-1"><button onClick={() => setAbaEstoque("loja")} className={`rounded-lg px-4 py-2 text-sm font-bold ${abaEstoque === "loja" ? "bg-white text-[#087443] shadow-sm" : "text-[#62766b]"}`}><Store size={16} className="mr-1.5 inline"/>Loja</button><button onClick={() => setAbaEstoque("cozinha")} className={`rounded-lg px-4 py-2 text-sm font-bold ${abaEstoque === "cozinha" ? "bg-white text-[#087443] shadow-sm" : "text-[#62766b]"}`}><Package size={16} className="mr-1.5 inline"/>Cozinha</button></div>{abaEstoque === "loja" ? <><p className="mb-4 text-sm text-[#62766b]">Saldo de marmitas prontas para venda. A produção não altera este saldo automaticamente.</p><div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{marmitas.map((produto) => <article key={produto.id} className="rounded-2xl border border-[#dbe7dd] bg-white p-4"><h3 className="font-bold">{produto.nome}</h3><div className="mt-4 grid grid-cols-3 gap-2 text-center text-xs"><div className="rounded-lg bg-[#f3f6f3] p-2"><b>{produto.estoque_200g ?? 0}</b><br/>200g</div><div className="rounded-lg bg-[#f3f6f3] p-2"><b>{produto.estoque_300g ?? 0}</b><br/>300g</div><div className="rounded-lg bg-[#f3f6f3] p-2"><b>{produto.estoque_400g ?? 0}</b><br/>400g</div></div></article>)}</div></> : <><p className="mb-4 text-sm text-[#62766b]">Controle ingredientes e marque rapidamente o que precisa ser reposto.</p><FormularioEstoque salvar={adicionarEstoque}/><div className="mt-5 grid gap-3 md:grid-cols-2">{(estoque as any[]).map((item) => <article key={item.id} className={`rounded-2xl border p-4 ${Number(item.quantidade_atual) <= Number(item.quantidade_minima) ? "border-amber-300 bg-amber-50" : "border-[#dbe7dd] bg-white"}`}><div className="flex justify-between gap-3"><h3 className="font-bold">{item.ingrediente}</h3><span className="text-sm font-black text-[#087443]">{item.quantidade_atual} {item.unidade}</span></div><p className="mt-2 text-xs text-[#62766b]">Mínimo: {item.quantidade_minima} {item.unidade}</p></article>)}{(estoque as any[]).length === 0 && <EstadoVazio texto="Ainda não há ingredientes cadastrados."/>}</div></>}</section>}
      </main>
    </div>
