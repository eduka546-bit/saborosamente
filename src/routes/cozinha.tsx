      ...
      {modal === "receita" && (
        <ReceitaModal
          ...
          salvar={async (modo: string, linhas: any[], montagem: MontagemLinha[]) => {
            ...
            const preparacoesFicha = linhas.filter((x) => x.preparacao_id).map((x) => ({ id: x.preparacao_id, nome: (preparacoes as any[]).find((p:any) => p.id === x.preparacao_id)?.nome || "Preparação" }));
...
function ReceitaModal({ produto, receita, linhasIniciais, montagemInicial, ingredientes, preparacoes = [], custoIng, criarIngrediente, fechar, salvar }: any) {
  ...
  const custoLinha = (x:ReceitaLinha,t:Tamanho) => x.preparacao_id ? n(x[`gramas_${t}` as keyof ReceitaLinha]) * n(custoPrep(x.preparacao_id)) : qCorreta(n(x[`gramas_${t}` as keyof ReceitaLinha]),ingredientes.find((a:any)=>a.id===x.ingrediente_id))*custoIng(ingredientes.find((a:any)=>a.id===x.ingrediente_id));
