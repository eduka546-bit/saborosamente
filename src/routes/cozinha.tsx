...
function ReceitaModal({ produto, receita, linhasIniciais, montagemInicial, ingredientes, preparacoes = [], itensPreparacao, custoIng, custoPrep, criarIngrediente, fechar, salvar }: any) {
  ...
  const preparacoesFicha = linhas.filter((x) => x.preparacao_id).map((x) => preparacoes.find((p:any) => p.id === x.preparacao_id)).filter(Boolean);
  const formatarGramas = (v: unknown) => n(v).toLocaleString("pt-BR", { maximumFractionDigits: 3 });
...
function TabelaCustos({linhas,ingredientes,nome,custoLinha,custoPrep,formatarGramas}:any){return <div className="overflow-x-auto rounded-2xl border border-[#dbe7dd]"><div className="min-w-[700px]"><div className="grid grid-cols-[minmax(220px,1fr)_100px_125px_125px_125px] gap-2 bg-[#edf5e6] px-3 py-3 text-xs font-bold uppercase tracking-wide text-[#527164]"><span>Componente</span><span>Custo/kg</span><span>200 g</span><span>300 g</span><span>400 g</span></div>{linhas.map((x:any,i:number)=>{const item=ingredientes.find((a:any)=>a.id===x.ingrediente_id);const custoKg=x.preparacao_id?valor(n(custoPrep(x.preparacao_id))*1000):item?valor(n(item.custo_por_kg)):"—";return <div key={i} className="grid grid-cols-[minmax(220px,1fr)_100px_125px_125px_125px] gap-2 border-t border-[#e2ebe3] bg-white px-3 py-3 text-sm"><b>{nome(x)}</b><span>{custoKg}</span>{TAMANHOS.map(t=><span key={t.id}>{formatarGramas(x[`gramas_${t.id}`])} g · <b>{valor(custoLinha(x,t.id))}</b></span>)}</div>})}</div></div>}
