...
const imprimirElemento = (id: string, titulo: string) => {
  ...
  janela.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${titulo}</title><style>
    @page{size:A4;margin:7mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#173a2d;margin:0;font-size:9px;line-height:1.2}h1,h2,h3,h4,p{margin-top:0;margin-bottom:3px}button{display:none!important}.grid{display:grid;gap:4px}.flex{display:flex}.flex-wrap{flex-wrap:wrap}.justify-between{justify-content:space-between}.items-center{align-items:center}.items-start{align-items:flex-start}.gap-2,.gap-3,.gap-4,.gap-5{gap:4px}.mb-2,.mb-3,.mb-4,.mb-5{margin-bottom:5px}.mt-1,.mt-2,.mt-3,.mt-4{margin-top:4px}.p-3,.p-4{padding:6px}.px-3{padding-left:6px;padding-right:6px}.py-1,.py-2{padding-top:3px;padding-bottom:3px}.rounded-xl,.rounded-2xl{border-radius:5px}.border{border:1px solid #dbe7dd}.bg-white{background:#fff}.bg-\[\#edf5e6\],.bg-\[\#f4f7f4\],.bg-\[\#f4f8f4\]{background:#f3f7f3}.bg-\[\#173a2d\]{background:#fff!important;color:#173a2d!important;border:1px solid #173a2d}.text-white,.text-white\/70{color:#173a2d!important}.text-\[\#087443\]{color:#087443}.text-\[\#527164\],.text-\[\#62766b\]{color:#52695f}.font-bold{font-weight:700}.font-black{font-weight:800}.text-xs{font-size:8px}.text-sm{font-size:9px}.text-lg{font-size:11px}.text-xl{font-size:13px}.text-2xl{font-size:15px}.uppercase{text-transform:uppercase}.whitespace-pre-line{white-space:pre-line}article{break-inside:avoid;page-break-inside:avoid}section{break-inside:auto;page-break-inside:auto}.page-break-before{break-before:page;page-break-before:always}[data-screen-only]{display:none!important}.print-only{display:block!important}.sm\:grid-cols-2,.md\:grid-cols-2,.lg\:grid-cols-2{grid-template-columns:repeat(2,minmax(0,1fr))}.sm\:grid-cols-3,.lg\:grid-cols-3{grid-template-columns:repeat(3,minmax(0,1fr))}img{max-width:100%}
...
function FichaProducaoDiaModal({ dataProducao, producoes, produtos, receitas, montagemPorReceita, separar, fechar }: any) {
  ...
  const pratos = Array.from(grupos.entries()).map(([produtoId, linhas]) => {
    ...
    return { produto, q, montagem, montagemTotal, total: q["200"] + q["300"] + q["400"] };
